...
while not done:
    ...
    train_tensors = paths_to_tensor(img_folder, X_train_parts).astype('float32')/255
    ...
    np.save('./bottleneck_features/Resnet50_train_{}'.format(num_files), train_features)
    np.save('./bottleneck_features/Resnet50_train-labels_{}'.format(num_files), y_train_parts)

    print("Successfully saved train bottleneck features for Resnet50_train_{}".format(i))
    prev_i = i
